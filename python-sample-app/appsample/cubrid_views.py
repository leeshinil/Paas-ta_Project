...
def org_chart(request, org_id):
    ...
    if request.method == 'GET':
        ...
        cursor.execute('SELECT * FROM ORG_TBL WHERE id = ?',org_id)
        ...
        response_dict.update({"groups":type_changed_groups}, status=200)
...
def org_list_detail(request, org_id):
    ...
    #2.2.5. 조직 수정
        ...
        cursor.execute(sql)
        rabbitmq_views.sendOrg(org_id,'cubrid','ORG_UPDATED')
    ...
    #2.2.6. 조직 삭제
        ...
        rabbitmq_views.sendOrg(org_id,'cubrid','ORG_DELETED')
...
def make_query(method, table_name, **kwargs):
    ...
    if method == 'POST':
        keys = data.keys()
        kString = ",".join(["%s" %k.encode('utf8') for k in keys])
        ...
        for v in values:
            if isinstance(v, int):
                ...
                sql_values += "".join(["'%s'," %v.encode('utf8')])
